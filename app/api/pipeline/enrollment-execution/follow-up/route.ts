...
import type { EnrollmentExecutionFollowUpInput } from '@/lib/pipeline/execution';
import {
  normalizeNullableString,
  normalizeNullableTimestamp,
} from '@/lib/pipeline/executionValidation';
import { appendEnrollmentExecutionFollowUpActivity } from '@/lib/pipeline/executionActivity';
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as EnrollmentExecutionFollowUpInput;
    ...
    const updatedMetadata = appendEnrollmentExecutionFollowUpActivity(
    ...
    console.error('POST /api/pipeline/enrollment-execution/follow-up failed', error);

    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Unexpected error while logging enrollment execution follow-up.' },
      { status: 500 },
    );
  }
}