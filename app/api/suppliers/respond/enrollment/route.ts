...
const pipelineId = extractStringField(payload, 'pipelineId');
    const enrollmentAttemptId = extractStringField(payload, 'enrollmentAttemptId');
    const enrollmentExecutionId = extractStringField(payload, 'enrollmentExecutionId');

    if (!pipelineId || !enrollmentAttemptId) {
      return badRequest('Supplier inbound request is missing enrollment linkage');
    }

    const ingestRes = await fetch(
      new URL('/api/integrations/enrollment/result', req.url).toString(),
      {
        method: 'POST',
    ...
    if (!ingestRes.ok) {
      return NextResponse.json(
        { error: ingestData.error ?? 'Failed to ingest enrollment response' },
    ...
    return NextResponse.json({
      message: 'Supplier enrollment response processed successfully.',
      result: ingestData,
    });
  } catch (error) {
    console.error('suppliers/respond/enrollment error', error);
