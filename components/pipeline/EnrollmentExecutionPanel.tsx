...
export default function EnrollmentExecutionPanel({
  pipelineId,
  isClosed,
  enrollmentOptions,
  executions,
  ...
  async function startIntegration() {
    if (!selectedEnrollmentId) {
      setError('Select an enrollment attempt first.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/integrations/enrollment/start', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          pipelineId,
          enrollmentAttemptId: selectedEnrollmentId,
          enrollmentExecutionId: selectedExecutionId || null,
      ...
      if (!res.ok) {
        setError(data.error ?? 'Failed to start enrollment integration.');
        return;
      }

      const ui = data.ui;
      const message = ui?.message ?? 'Enrollment integration started.';
      setInfo(message);
      ...
      setError('Unexpected error starting enrollment integration.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5 space-y-4">
      <div>
        <h3 className="text-white font-semibold text-lg">Enrollment Execution</h3>
        <p className="text-zinc-400 text-sm">
          Start supplier enrollment workflows and track external execution state.
        </p>
      </div>
      ...
      {!hasEnrollments ? (
        ...
        <>
          ...
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-700 text-white p-2 rounded text-sm disabled:opacity-50"
          >
            {loading ? 'Starting...' : 'Start Enrollment Execution'}
          </button>
